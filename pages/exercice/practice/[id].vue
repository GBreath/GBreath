...
function startBreathing() {
  ...
  breathingStep({
    stepTime: 5,
    prefixText: $t("breathing.practice.breathing_starting_in"),
    nextStep: inhale,
  });
}
...
function inhale() {
  ...
  breathingStep({
    stepTime: exercices[id].steps[0],
    prefixText: $t("breathing.practice.inhale_for"),
    nextStep: hold,
  });
}
...
function hold() {
  ...
  breathingStep({
    stepTime: exercices[id].steps[1],
    prefixText: $t("breathing.practice.hold_for"),
    nextStep: exhale,
  });
}
...
function exhale() {
  ...
  breathingStep({
    stepTime: exercices[id].steps[2],
    prefixText: $t("breathing.practice.exhale_for"),
    nextStep: rest,
  });
}
...
function rest() {
  ...
  currentAnimation.value = "";
  breathingStep({
    stepTime: exercices[id].steps[3],
    prefixText: $t("breathing.practice.take_a_rest_for"),
    nextStep: inhale,
  });
...
<template>
  <div class="page-content !gap-8 !items-center">
    <top-bar :title="$ot(exercise.name, $i18n.locale)" />

    <template v-if="!isFinished">
      <h3>
        <strong>{{ repeatTime - alreadyRun }}</strong>
        {{ $t("breathing.practice.breaths_to_go") }}
      </h3>
      <div class="outside">
        <div
      ...
      <h3 class="medium">{{ visualReturn }}</h3></template
    >
    <template v-else>
      <section class="completed">
        ...
        <h3>
          <strong
            >{{
              $t("breathing.practice.your_breathing_session_is_complete")
            }}!</strong
          >
        </h3>
        <br />
        <p>{{ $t("breathing.practice.i_hope_you_are_feeling_better") }}.</p>
        <svg
